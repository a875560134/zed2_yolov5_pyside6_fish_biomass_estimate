...
class WandbLogger:
    ...
    def __init__(self, opt, name, run_id, data_dict, job_type='Training'):
        ...
        if self.wandb_run:
            ...
            colorstr('wandb: ')
            print("{prefix}Install Weights & Biases for YOLOv5 logging with 'pip install wandb' (recommended)")
    ...
    @staticmethod
    def download_dataset_artifact(path, alias):
        """
        """
        if isinstance(path, str) and path.startswith(WANDB_ARTIFACT_PREFIX):
            artifact_path = Path(remove_prefix(path) + ":" + alias)
    ...
    @staticmethod
    def download_model_artifact(opt):
        """
        """
        if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
            model_artifact = wandb.use_artifact(remove_prefix(opt.resume) + ":latest")
    ...
    @staticmethod
    def create_dataset_table(dataset, class_to_id, name='dataset'):
        """
        """
