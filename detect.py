...
def run(weights='yolov5n.pt',  # model.pt path(s)
    ...
    classify = False
    if classify:
        modelc = load_classifier(name='resnet50')  # initialize
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            if webcam:  # batch_size >= 1
                p, s, im0, frame = path[i], '{i}: ', im0s[i].copy(), dataset.count
            else:
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if save_crop:
                            save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / '{p.stem}.jpg', BGR=True)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./pt/best.pt', help='model.pt path(s)')
...
def main(opt):
    """
    """
    print(colorstr('detect: ') + ', '.join('{k}={v}' for _, _ in vars(opt).items()))
