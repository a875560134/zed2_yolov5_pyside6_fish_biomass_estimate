...
def run(weights='./yolov5s.pt',  # weights path
    ...
    if 'torchscript' in include or 'coreml' in include:
        colorstr('TorchScript:')
        try:
            print('\n{prefix} starting export with torch {torch.__version__}...')
            f = weights.replace('.pt', '.torchscript.pt')  # filename
            ts = torch.jit.trace(model, img, strict=False)
            (optimize_for_mobile(ts) if optimize else ts).save(f)
            print('{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
        except Exception:
            print('{prefix} export failure: {e}')
    ...
    if 'onnx' in include:
        ...
        try:
            ...
            print('{prefix} starting export with onnx {onnx.__version__}...')
            f = weights.replace('.pt', '.onnx')  # filename
            torch.onnx.export(model, img, f, opset_version=opset_version,
            ...
            # Checks
            model_onnx = onnxsim.load(f)  # load onnx model
            onnxsim.checker.check_model(model_onnx)  # check onnx model
            ...
            if simplify:
                try:
                    ...
                    print('{prefix} simplifying with onnx-simplifier {onnxsim.__version__}...')
                    model_onnx, check = onnxsim.simplify(
                        model_onnx,
                        dynamic_input_shape=dynamic,
                        input_shapes={'images': list(img.shape)} if dynamic else None)
                    assert check, 'assert check failed'
                    onnxsim.save(model_onnx, f)
                except Exception:
                    print('{prefix} simplifier failure: {e}')
            print('{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
        except Exception:
            print('{prefix} export failure: {e}')
    ...
    if 'coreml' in include:
        ...
        try:
            ...
            print('{prefix} starting export with coremltools {ct.__version__}...')
...
def main(opt):
    ...
    """
    set_logging()
    print(colorstr('export: ') + ', '.join('{k}={v}' for _, _ in vars(opt).items()))
    run(**vars(opt))
