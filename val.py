...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if plots and batch_i < 3:
            f = save_dir / 'val_batch{batch_i}_labels.jpg'  # labels
            Thread(target=plot_images, args=(img, targets, paths, f, names), daemon=True).start()
            f = save_dir / 'val_batch{batch_i}_pred.jpg'  # predictions
    ...
    if not training:
        var = (batch_size, 3, imgsz, imgsz)
        print('Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {shape}' % t)
    ...
    if save_json and len(jdict):
        ...
        pred_json = str(save_dir / "{w}_predictions.json")  # predictions json
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='yolov5n.pt', help='model.pt path(s)')
...
def main(opt):
    ...
    """
    set_logging()
    print(colorstr('bright_blue', 'bold', 'val: ') + ', '.join('{k}={v}' for _, _ in vars(opt).items()))
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        for w in opt.weights if isinstance(opt.weights, list) else [opt.weights]:
            f = 'study_{Path(opt.data).stem}_{Path(w).stem}.txt'  # filename to save to
            y = []  # y axis
            for i in x:  # img-size
                print('\nRunning {f} point {i}...')
