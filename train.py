...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    LOGGER.info(colorstr('hyperparameters: ') + ', '.join('{k}={v}' for _, _ in hyp.items()))
    ...
    if RANK in [-1, 0]:
        # TensorBoard
        if not evolve:
            colorstr('tensorboard: ')
            LOGGER.info("{prefix}Start with 'tensorboard --logdir {opt.project}', view at http://localhost:6006/")
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        ...
        model = Model(cfg, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    with torch_distributed_zero_first(RANK):
    ...
    LOGGER.info("Scaled weight_decay = {hyp['weight_decay']}")
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls, hyp=hyp,
                                       cache=opt.cache_images and not noval, rect=True, pad=0.5,
    ...
    LOGGER.info('Image sizes {imgsz} train, {imgsz} val\n'
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                # Plot
                if plots and ni < 3:
                    f = save_dir / 'train_batch{ni}.jpg'  # filename
    ...
    # end training -----------------------------------------------------------------------------------------------------
    if RANK in [-1, 0]:
        LOGGER.info('{epoch - start_epoch + 1} epochs completed in {(time.time() - t0) / 3600:.3f} hours.\n')
        if plots:
            plot_results(save_dir=save_dir)  # save as results.png
            if loggers['wandb']:
                files = ['results.png', 'confusion_matrix.png', *['{x}_curve.png' for _ in ('F1', 'PR', 'P', 'R')]]
...
def main(opt):
    ...
    set_logging(RANK)
    if RANK in [-1, 0]:
        print(colorstr('train: ') + ', '.join('{k}={v}' for _, _ in vars(opt).items()))
        check_git_status()
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        LOGGER.info('Resuming training from {ckpt}')
    else:
    ...
    # Evolve hyperparameters (optional)
        ...
        if opt.bucket:
            os.system('gsutil cp gs://{opt.bucket}/evolve.txt .')  # download evolve.txt if exists
        ...
        print('Hyperparameter evolution complete. Best results saved as: {yaml_file}\n'
